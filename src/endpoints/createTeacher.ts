...
export const teacherCreate = async (
  req: Request,
  res: Response
): Promise<void> => {
  let errorCode = 400;
  try {
    ...
    const existingTeacherId = await connection("DOCENTE")
      .where("id", input.id)
      .first();

    if (existingTeacherId) {
      throw new Error("Este ID já foi cadastrado. Registre um ID diferente.");
    }
    ...
    res
      .status(201)
      .send({ message: `Docente ${input.nome}, criado com sucesso!` });
  } catch (error: any) {
